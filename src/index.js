...
const supportFooter = `Soutenir : ${supportLink}`;
...
async function handleConfession(message) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0xb07bff)
    .setFooter({ text: `Confession #${confessionId} • ${supportFooter}` })
    .setTimestamp();
...
function createQuizThemeEmbed() {
  ...
  return new EmbedBuilder()
    ...
    .setColor(0x5dade2)
    .setFooter({ text: `Réagissez pour voter (égalité → aléatoire). • ${supportFooter}` })
    .setTimestamp();
}
...
function createQuizQuestionEmbed(themeName, questionIndex, question) {
  ...
  return new EmbedBuilder()
    ...
    .setColor(0x45b39d)
    .setFooter({ text: `Temps limité : ${quizQuestionDurationMs / 1000}s • ${supportFooter}` })
    .setTimestamp();
}
...
async function handleSupportCommand(message) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0xf4b5b5)
    .setFooter({ text: 'Votre support aide à maintenir et améliorer le bot.' })
    .setTimestamp();
